...
## Покрытие: что уже считается, а что ждёт данных

`GET /api/economy/catalog` отдаёт вместе с каталогом разбивку `coverage`, посчитанную по фактическому
списку событий из адаптеров игр (`/api/ingestion/adapters`):

- `fromEvents` — метрики, которые заполнятся сразу после потока принимаемых событий (18 из 40);
- `needEvents` — метрики, которым нужны эмиттеры, которых в играх ещё нет (9 из 40):
  `FeeCharged`, `MarketOrderCompleted`, `WalletConnected`, `Withdrawal`, `SecurityEvent`, `BridgeIn`, `BridgeOut`;
- `needConfig` — метрики, которым нужна финансовая конфигурация студии (13 из 40): `circulating`,
  `maxSupply`, `revenueUsd`, `costsUsd`, `stablecoinRevenueUsd`, `cosmeticRevenueUsd`, `treasuryBalance`,
  `dailyBurn`, `burnCadenceDays`, `liquidityUsd`.

### Период финансовых фактов

Выручка, расходы и их разбивка (`WATCHTOWER_ECONOMY_REVENUE_USD`, `COSTS_USD`,
`STABLECOIN_REVENUE_USD`, `COSMETIC_REVENUE_USD`) задаются **за календарный месяц — 30 дней**
(`CONFIG_PERIOD_DAYS`) и приводятся к выбранному окну пропорционально дням:

```
значение_за_окно = значение_за_30_дней × дни_окна / 30
```

Следствия, зафиксированные тестами:

- на окне 30 дней показанное число равно присланному студии (никакого искажения);
- `stablecoin_share` и `cosmetic_share` от окна не зависят — приведение не ломает пропорции;
- `arpdau` = выручка за окно / активные кошельки / дни окна, то есть USD на игрока в день и тоже не зависит от длины окна;
- такие метрики помечаются `quality: "partial"` и в `source` указывают период конфигурации: расчетная величина не выдаётся за измеренную за окно.

Значения supply, казны, ликвидности и капитализации — на текущую дату (они не масштабируются).

Восемь метрик считаются только из этих фактов и **видны даже при пустом потоке событий**
(`gross_revenue`, `dividend_pool_coverage`, `stablecoin_share`, `cosmetic_share`, `cap_utilization`,
`treasury_runway_days`, `liquidity_to_mcap`, `price_impact_1k`). Они помечаются `quality: "partial"`
и получают `note` «Из подтверждённых финансовых фактов студии: события игр для этой метрики не требуются» —
заявленный факт не выдаётся за измеренную активность. Метрики активности без событий остаются
`unavailable` с причиной, индекс здоровья из одних фактов не строится.

Форму для сбора цифр у финансовой команды см. `prompts/studio-os/PROMPT_STUDIO_FINANCE_CONFIG.md`.

Полный инвентарь того, что снимаем с игр и что считаем, — `docs/METRICS_INVENTORY.md`.
