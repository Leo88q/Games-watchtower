...
## Регрессии сборки образа, найденные CI (продолжение F-020)

Первый прогон CI на PR показал, что образ **собирался не полностью** — и это не косметика:

1. `vite.config.js` собирает два интерфейса (`index.html` и `ios.html`), а в стадию сборки
   копировался только `index.html` → `vite build` падал.
2. `server/index.js` и `server/analytics/live-model.js` импортируют реестр игр из `src/data/registry.js`,
   но `src/` в runtime-стадию не копировался → контейнер падал на старте (`ERR_MODULE_NOT_FOUND`),
   даже если бы сборка прошла.
3. `.dockerignore` исключал `prompts/`, поэтому раздел «Промпты» в образе был пустым.

Исправлено, и теперь это ловят три независимых уровня: шаг CI «образ запускается и отвечает на
`/api/health`» (реальный запуск контейнера с проверкой статики, API и 401 на write без токена),
тест `test:docs` («Dockerfile копирует всё, что нужно сборке и серверу») и локальная проверка
`npm run verify`.
