...
const FIX_TOOLCHAIN = flag('fix-toolchain')   // разрешает rustup toolchain install недостающего пина
...
/** Одна строка вывода команды ('' если команды нет) — для самодиагностики toolchain. */
const line = (cmd) => {
  const r = spawnSync('/bin/sh', ['-c', cmd], { encoding: 'utf8', timeout: 30000 })
  const first = (r.stdout || '').trim().split('\n')[0] || ''
  // head в пайплайне обнуляет код возврата, поэтому «command not found» ловим по тексту.
  return r.status === 0 && !/not found|No such file|is not recognized/i.test(first) ? first : ''
}
...
/** Заглушки обычно делают повторяющимися хвостами (CgInv1111…, SixSec1111…). Валидный адрес так не выглядит. */
function isPlaceholder(id) {
  const m = String(id).match(/(.)\1{4,}/)
  return m ? `${m[0].length} повторов символа «${m[1]}»` : null
}
...
function findAnchorWorkspace(repoDir) {
  ...
  return hits.sort((a, b) => (path.basename(a) === 'Anchor.toml' ? -1 : 1) - (path.basename(b) === 'Anchor.toml' ? -1 : 1))
    .map((toml) => {
      const dir = path.dirname(toml)
      return {
        toml, dir,
        exactName: path.basename(toml) === 'Anchor.toml',
        hasCrate: existsSync(path.join(dir, 'Cargo.toml')) || existsSync(path.join(dir, 'programs')),
      }
    })
}
...
function processRepo(repo) {
  ...
  for (const ws of workspaces) {
    ...
    if (!ws.hasCrate) {
      warn(`${rel}: ${path.basename(ws.toml)} без Cargo.toml/programs/ — это спецификация, собирать и деплоить нечего`)
      record(repo, 'anchor', 'skip', 'нет Rust-крейта')
      continue
    }
    if (!ws.exactName) {
    ...
    toolchainStatus(repo, ws)
    const before = DRY ? '' : gitOut(dir, 'git status --porcelain')
    if (!SKIP_TESTS) {
      const built = exec(repo, `anchor build (${rel})`, 'anchor build', { cwd: ws.dir })
      exec(repo, `anchor test (${rel})`, 'anchor test', { cwd: ws.dir })
      if (built.ok && !DRY) {
        const after = gitOut(dir, 'git status --porcelain')
        if (after !== before) {
          bad('anchor build изменил рабочее дерево (обычно он переписывает program id по keypair из target/deploy):')
          console.log(dim(gitOut(dir, 'git diff --stat').split('\n').slice(0, 8).map((l) => `      ${l}`).join('\n')))
          console.log(dim([
            '      два варианта, выбрать сознательно:',
            '      · id из репозитория верны → снести локальные keypair и пересобрать: rm -rf target/deploy && anchor build',
            '      · верны новые id → проверить, что публичные ключи совпадают: solana-keygen pubkey target/deploy/<program>-keypair.json',
            '      в коммит без этого не пускать, а деплой заблокирован до зелёного прогона',
          ].map((l) => `      ${l}`).join('\n').split('\n').map((l) => `      ${l.trim()}`.slice(0, 200)).join('\n')))
          record(repo, 'чистота дерева после anchor', 'fail', 'Anchor.toml/lib.rs переписаны')
        } else ok('дерево после anchor build не изменилось')
      }
...
/**
 * Версии и пин Rust-тулчейна. Типичный отказ на этой студии:
 *   error: no such command: `+1.89.0-sbpf-solana-v1.54`
 * — anchor зовёт `cargo +<пин> build-sbf`, а такого тулчейна в rustup нет (или его только что сняли).
 */
function toolchainStatus(repo, ws) {
  step(repo, 'toolchain')
  const tc = tomlSection(readFileSync(ws.toml, 'utf8'), 'toolchain')
  const pin = tc.build || tc.check || ''
  const found = {
    anchor: line('anchor --version 2>&1 | head -1'),
    avm: line('avm current 2>&1 | head -1'),
    rustup: line('rustup --version 2>&1 | head -1'),
    solana: line('solana --version 2>&1 | head -1'),
    sbf: line('cargo build-sbf --version 2>&1 | head -1'),
  }
  for (const [k, v] of Object.entries(found)) console.log(`  ${k.padEnd(7)} ${v || c('33', 'не определён')}`)
  console.log(`  ${'пин'.padEnd(7)} ${pin || dim('не задан в [toolchain]')}`)
  if (DRY) { record(repo, 'toolchain', 'dry'); return }

  const pkg = readJson(path.join(ws.dir, 'package.json'))
  const lib = Object.entries({ ...(pkg?.dependencies || {}), ...(pkg?.devDependencies || {}) })
    .find(([k]) => k === '@coral-xyz/anchor')?.[1]
  if (lib && found.anchor) {
    const cli = (found.anchor.match(/(\d+\.\d+\.\d+)/) || [])[1]
    const want = (String(lib).match(/(\d+\.\d+\.\d+)/) || [])[1]
    if (cli && want && cli !== want) {
      warn(`версии не совпадают: CLI anchor ${cli} vs @coral-xyz/anchor ${lib} — это и есть источник «don't match» в логах сборки`)
      record(repo, 'версии anchor', 'warn', `CLI ${cli} ≠ lib ${want}`)
    }
  }
  if (!found.rustup) {
    bad('rustup не найден — SBF-сборка невозможна (brew install rustup-init && rustup default stable)')
    record(repo, 'rustup', 'fail', 'не установлен')
    return
  }
  const installed = line('rustup toolchain list')
  if (pin && !installed.split('\n').some((l) => l.startsWith(pin))) {
    if (!FIX_TOOLCHAIN) {
      bad(`тулчейна ${pin} (пин из [toolchain]) в rustup нет — отсюда «no such command: +${pin}»`)
      console.log(dim(`      починить: rustup toolchain install ${pin} --profile minimal   (или прогон с --fix-toolchain)`))
      record(repo, 'toolchain-пин', 'fail', `${pin} не установлен`)
    } else {
      exec(repo, `rustup toolchain install ${pin}`, `rustup toolchain install ${pin} --profile minimal`)
      const again = line('rustup toolchain list')
      if (again.split('\n').some((l) => l.startsWith(pin))) ok(`${pin} установлен`)
      else bad(`после установки ${pin} всё равно не виден — пин недостижим из rustup: обновите agave/solana CLI (agave-install update) и повторите`)
    }
  } else if (pin) ok(`тулчейн ${pin} на месте`)
  if (!found.sbf) { warn('cargo build-sbf не отвечает: сборка программ не пройдёт (нужен agave/solana CLI в PATH)'); record(repo, 'cargo build-sbf', 'warn', 'не вызывается') }
}
...
function checkClusterConfig(repo, ws, rel) {
  ...
  for (const [name, id] of entries) {
    const verdict = checkProgramId(id)
    const ghost = verdict === 'ok' ? isPlaceholder(id) : null
    if (verdict === 'ok' && !ghost) ok(`${name} → ${id}`)
    else if (ghost) { warn(`${name} → ${id}: выглядит как заглушка (${ghost}) — после реального деплоя сверить с pubkey keypair`) ; record(repo, `program id ${name}`, 'warn', ghost) }
    else { bad(`${name} → ${id} : ${verdict} — невалидный адрес`); record(repo, `program id ${name}`, 'fail', verdict); invalid += 1 }
  }
...
function deployStage(repo, ws, rel, ids) {
  ...
  for (const [name, id] of Object.entries(ids)) {
    ...
    else if (isPlaceholder(id)) blockers.push(`program id ${name} похож на заглушку (${isPlaceholder(id)})`)
  }
