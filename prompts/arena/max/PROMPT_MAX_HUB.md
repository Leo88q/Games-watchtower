...
**Кому адресован.** Инженеру платформы (хаба) — человеку или AI-агенту Arena, который работает
в сессии на репозитории `Leo88q/Games-watchtower`. Это **единственный** промпт набора, которому
разрешено менять хаб: реестр, отчёты, сервер, панели, контракты.

**Кому НЕ адресован.** Сессиям на репозиториях игр и приложений (`ares1`, `aof`, `neon-relay`,
`guttercaps`, `talkchart-traffic-generator`, `investor`) — у них свои промпты `PROMPT_MAX_*`,
а хаб они читают только на чтение (см. `prompts/arena/00_HUB_CONTRACT.md` §1).

**Кто потребитель результата.**

| Потребитель | Что получает |
|---|---|
| Игры (4) | `/api/ecosystem/status`, честный реестр, реальные program id в ENV, канонический словарь событий |
| Приложение `investor` | `/api/ecosystem/report` и честные метрики вместо mock |
| Оператор студии (владелец репозитория) | отчёт покрытия L0–L4, список оставшихся пробелов, SLO-дашборд |
| Инвестор | сводные отчёты без mock, с бейджами `dataQuality` и ссылками на источники |

**Когда запускать.** Первым (волна W0) — до игровых MAX-промптов: без честного реестра и статусов
игры не смогут подтвердить свой уровень. Повторно — в конце, для сводки «уровень до → уровень после»
по каждому tenant'у и списка пробелов.

**Соотношение с минимальным промптом хаба.** `PROMPT_ARENA_WATCHTOWER_HUB.md` = минимум
(целостность отчётов, реестр, приём 5 tenant'ов, смоук). Этот файл = максимум и включает P0-часть
минимума целиком, поэтому при одном прогоне достаточно запускать MAX.
